...
Mesh ImportedModel::processMesh(aiMesh *mesh, const aiScene *scene)
{
    ...
    for (unsigned int i = 0; i < mesh->mNumVertices; i++)
    {
        ...
        if (mesh->mTextureCoords[0]) { // does the mesh contain normal coordinates?
    ...
    if (mesh->mMaterialIndex >= 0)
    {
        ...
        std::vector<ImageTexture> diffuseMaps = loadMaterialTextures(material,
            aiTextureType_DIFFUSE, "texture_diffuse");
        textures.insert(textures.end(), diffuseMaps.begin(), diffuseMaps.end());
        std::vector<ImageTexture> specularMaps = loadMaterialTextures(material,
...
std::vector<ImageTexture> ImportedModel::loadMaterialTextures(aiMaterial *mat, aiTextureType type, std::string typeName)
{
    ...
    for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
    {
        ...
        ImageTexture texture(str.C_Str(), typeName);
